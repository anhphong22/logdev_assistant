...
function gradioLoaded(mutations) {
    for (var i = 0; i < mutations.length; i++) {
        if (mutations[i].addedNodes.length) {
            ...
            chatbot = document.querySelector('#logdev_chatbot');
            chatbotWrap = document.querySelector('#logdev_chatbot > .wrap');
...
function addLogdevButton(botElement) {
    var rawMessage = null;
    var mdMessage = null;
    rawMessage = botElement.querySelector('.raw-message');
    mdMessage = botElement.querySelector('.md-message');
    if (!rawMessage) {
        var buttons = botElement.querySelectorAll('button.logdev-btn');
    ...
    copyButton.classList.add('logdev-btn');
    ...
    toggleButton.classList.add('logdev-btn');
        ...
        if (mmutation.type === 'childList') {
            for (var node of mmutation.addedNodes) {
                if (node.nodeType === 1 && node.classList.contains('message') && node.getAttribute('data-testid') === 'bot') {
                    ...
                    saveHistoryHtml();
                    document.querySelectorAll('#logdev_chatbot>.wrap>.message-wrap .message.bot').forEach(addLogdevButton);
                    document.querySelectorAll('#logdev_chatbot>.wrap>.message-wrap .message.bot pre').forEach(addCopyCodeButton);
                }
            }
...
function saveHistoryHtml() {
    var historyHtml = document.querySelector('#logdev_chatbot > .wrap');
...
function loadHistoryHtml() {
    ...
    if (!historyLoaded) {
        ...
        var buttons = tempDiv.querySelectorAll('button.logdev-btn');
        for (var i = 0; i < buttons.length; i++) {
            buttons[i].parentNode.removeChild(buttons[i]);
        }
